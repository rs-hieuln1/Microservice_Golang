...
func (app *Config) logItem(w http.ResponseWriter, entry LogPayload) {
	fmt.Println("Log service url is")
	...
	fmt.Println("Log service url is", logServiceURL)
...
func (app *Config) authenticate(w http.ResponseWriter, a AuthPayload) {
	...
	fmt.Println("we made it here", response.StatusCode)
	// make sure we get back the correct status code
	if response.StatusCode == http.StatusUnauthorized {
		app.errorJSON(w, errors.New("invalid credentials"))
		return
	} else if response.StatusCode != http.StatusAccepted {
		app.errorJSON(w, fmt.Errorf("error calling auth service %d why %v", response.StatusCode, response))
		return
	}
...
func (app *Config) sendMail(w http.ResponseWriter, msg MailPayload) {
	...
	mailServiceURL := "http://mailer-service:3000/send"
...
func (app *Config) logEventViaRPC(w http.ResponseWriter, l LogPayload) {
	...
	payloadResp := jsonResponse{
		Error:   false,
		Message: resp,
	}

	err = app.writeJSON(w, http.StatusAccepted, payloadResp)
	if err != nil {
		app.errorJSON(w, err)
		return
	}
...
func (app *Config) logEventViaGRPC(w http.ResponseWriter, r *http.Request) {
	...
	logServiceAddress := "logger-service:50001"
	conn, err := grpc.Dial(logServiceAddress, grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		app.errorJSON(w, err)
		return
	}
